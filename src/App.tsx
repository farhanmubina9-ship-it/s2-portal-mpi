    ...
    <div className={`min-h-screen transition-colors duration-200 font-sans ${darkMode ? 'bg-gray-950 text-gray-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <header className={`sticky top-0 z-40 border-b backdrop-blur-md transition-colors ${darkMode ? 'bg-gray-900/90 border-gray-800' : 'bg-white/90 border-slate-200'}`}>
        <div className="max-w-md md:max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setSelectedCourseId(null)}>
            <div className="w-9 h-9 rounded-xl bg-emerald-600 text-white font-bold flex items-center justify-center text-lg shadow-sm">
              S2
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <h1 className="font-extrabold text-base leading-tight tracking-tight">Portal MPS2</h1>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 ${
                  ...
                  {isSyncedWithSupabase ? 'Cloud Sync' : 'Local Data'}
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-gray-400">Magister Pend. Islam</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{isLoggedInAdmin ? 'Kosma Active' : 'Kosma Admin'}</span>
            </button>

            {/* Mode Siang/Malam */}
            <button
              onClick={() => setDarkMode(!darkMode)}
              className={`p-2 rounded-xl border transition-all text-sm font-semibold flex items-center gap-1.5 ${
                darkMode 
            ...
            >
              ...
              <span className="text-xs hidden sm:inline">{darkMode ? 'Siang' : 'Malam'}</span>
            </button>
          </div>
        </div>

        {/* TOP TAB NAV (MAIN SCREEN) */}
        <div className="max-w-md md:max-w-3xl mx-auto px-4 flex border-t border-slate-200/60 dark:border-gray-800">
          <button
            onClick={() => {
              setActiveTab('jadwal');
              setSelectedCourseId(null);
            }}
            className={`flex-1 py-3 text-xs sm:text-sm font-bold border-b-2 text-center transition-colors flex items-center justify-center gap-1.5 ${
      ...
      {/* MAIN CONTAINER */}
      <main className="max-w-md md:max-w-3xl mx-auto px-4 py-5 space-y-6">
        ...
        {selectedCourse ? (
          <div className="space-y-4 animate-fadeIn">
            ...
            >
              ...
              <div className="mt-4 pt-3 border-t border-current/20 grid grid-cols-2 gap-2 text-xs font-semibold">
                <div className="flex items-center gap-1.5">
            ...
            {/* CONTENT TAB DETAIL */}
            {detailTab === 'info' && (
              <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'} space-y-4`}>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-extrabold text-sm tracking-wide uppercase text-emerald-600 dark:text-emerald-400">
                      Jumat (4 Mata Kuliah)
                    </h3>
                  </div>
                      ...
                      .map(course => {
                        ...
                        return (
                          <div
                            ...
                            className="p-4 rounded-2xl border transition-all duration-200 hover:scale-[1.01] cursor-pointer shadow-xs relative overflow-hidden flex flex-col justify-between"
                            style={{
                          ...
                          >
                            <div>
                              ...
                              <p className="text-[11px] font-medium mt-1 opacity-90">Dosen: {course.lecturer}</p>
                            </div>

                            <div className="mt-3 pt-2 border-t border-current/20 flex items-center justify-between text-[11px] font-semibold">
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-extrabold text-sm tracking-wide uppercase text-blue-600 dark:text-blue-400">
                      Sabtu (3 Mata Kuliah)
                    </h3>
                  </div>
            ...
            {activeTab === 'tugas' && (
              <div className="space-y-4">
                <h3 className="font-bold text-base flex items-center gap-2">
                  <FileText className="w-5 h-5 text-emerald-600" /> Ringkasan Seluruh Tugas Semester 2
                </h3>

                {allTasks.length === 0 ? (
                  <div className={`p-10 text-center rounded-2xl border ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'}`}>
                    <CheckCircle2 className="w-10 h-10 text-emerald-500 mx-auto mb-2 opacity-50" />
                    <p className="text-sm font-semibold text-slate-600 dark:text-gray-400">Tidak ada tugas terdaftar di semua mata kuliah.</p>
                  </div>
                ) : (
                  allTasks.map(task => (
                    <div 
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <h4 className="font-bold text-sm mt-2">{task.title}</h4>
                          <p className="text-xs text-slate-600 dark:text-gray-400 mt-1 leading-relaxed">{task.description}</p>
                          
                          <div className="flex items-center gap-3 text-xs font-medium text-slate-500 dark:text-gray-400 mt-2">
            ...
            {activeTab === 'admin' && (
              <div className="space-y-4">
                {!isLoggedInAdmin ? (
                  ...
                  <div className={`p-6 rounded-2xl border max-w-sm mx-auto space-y-4 text-center ${
                    darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'
                  }`}>
                  ...
                  <div className="space-y-5">
                    ...
                    <div className={`p-4 rounded-2xl border flex items-center justify-between ${
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-extrabold text-sm">Mode Pengelola (Kosma/Admin)</h3>
                          <p className="text-xs text-slate-500 dark:text-gray-400">Hak Akses: Penuh (Upload Multi PDF & Task Sync)</p>
                        </div>
                      </div>

                      <button
                        onClick={handleAdminLogout}
                        className="px-3 py-1.5 rounded-xl border border-rose-200 dark:border-rose-900 bg-rose-50 dark:bg-rose-950 text-rose-600 dark:text-rose-400 text-xs font-bold flex items-center gap-1.5 hover:bg-rose-100"
                      >
...
CREATE POLICY "Public access" ON mps2_store FOR ALL USING (true) WITH CHECK (true);`}
                    ...
                    <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'} space-y-3`}>
                      <h3 className="font-bold text-sm text-emerald-600 dark:text-emerald-400 flex items-center gap-2">
                        <Paperclip className="w-4 h-4" /> Kelola File PDF Terunggah Per Mata Kuliah (Max 2 File/Matkul)
                      </h3>
                    ...
                    <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'} space-y-4`}>
                      <div className="flex items-center justify-between border-b border-slate-100 dark:border-gray-800 pb-3">
                        <div>
                          <h3 className="font-bold text-sm text-emerald-600 dark:text-emerald-400 flex items-center gap-2">
                            <Upload className="w-4 h-4" /> Upload Sekaligus (3 Slot PDF Silabus)
                          </h3>
                      ...
                      <div className="space-y-3">
                        {bulkSlots.map((slot, index) => (
                          ...
                          >
                            <span className="px-2.5 py-1 bg-emerald-600 text-white rounded-lg text-xs font-bold self-start sm:self-center">
                              Slot #{index + 1}
                            </span>
                            ...
                            <input
                              ...
                              className="text-xs file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-emerald-100 file:text-emerald-700 dark:file:bg-emerald-950 dark:file:text-emerald-300 hover:file:bg-emerald-200 flex-1"
                            />
                          </div>
                        ))}
                      </div>
                    ...
                    <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'} space-y-4`}>
                      <h3 className="font-bold text-sm flex items-center gap-2 text-purple-600 dark:text-purple-400">
                        <Sparkles className="w-4 h-4" /> Upload PDF (Tambah File 1 / File 2) & Ekstraksi AI
                      </h3>
                    ...
                    <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'} space-y-3`}>
                      <h3 className="font-bold text-sm flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                        <UserPlus className="w-4 h-4" /> Atur Kontak Penanggung Jawab (PJ Matkul)
                      </h3>
